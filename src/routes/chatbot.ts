...
async function getIssuesContext(userId: string): Promise<string> {
  try {
    ...
    const bullets = rows
      .map(
        (r: any) =>
          `- ${r.issue_id}: ${r.issue_type} (severity: ${r.severity ?? "n/a"}, status: ${r.status})`
      )
      .join("\n");
...
async function getGlossaryBlock(userId: string): Promise<string> {
  try {
    const { rows } = await query(
      `
    ...
    const lines = rows
      .map(
        (r: any) =>
          `- ${r.term}: ${r.definition}${r.category ? ` [${r.category}]` : ""}`
      )
      .join("\n");
...
async function generateChatTitle(firstMessage: string): Promise<string> {
  try {
    ...
    const result = await model.generateContent(
      `Generate a concise, descriptive chat title (≤6 words) for this conversation:\n\n"${firstMessage}"`
    );
    const title = result.response.text().trim().replace(/['"]/g, "");
    return title.slice(0, 50) || "Chat Session";
  } catch {
    const s = firstMessage.toLowerCase();
    if (s.includes("compliance")) return "Compliance Discussion";
    if (s.includes("issue")) return "Issue Resolution";
    if (s.includes("data")) return "Data Analysis";
    return "Chat Session";
  }
}
...
// Helpers
// ───────────────────────────────────────────────────────────────────────────────
function sanitizeInput(s: unknown, max = 4000): string | null {
...
router.post("/sessions", requireAuth, async (req: AuthedReq, res) => {
  try {
    ...
    res.json({ session: result.rows[0] as ChatSession });
  } catch (error) {
...
router.get("/sessions", requireAuth, async (req: AuthedReq, res) => {
  try {
    ...
    res.json({ sessions: result.rows as ChatSession[] });
  } catch (error) {
...
router.get("/sessions/:sessionId/messages", requireAuth, async (req: AuthedReq, res) => {
  try {
    ...
    const result = await query(
      `SELECT id, role, content, timestamp, session_id, user_id
       FROM chat_messages
       WHERE session_id = $1
    ...
    res.json({ messages: result.rows as ChatMessage[] });
  } catch (error) {
...
router.post("/sessions/:sessionId/messages", requireAuth, async (req: AuthedReq, res) => {
  try {
    ...
    const countRes = await query("SELECT COUNT(*)::int AS c FROM chat_messages WHERE session_id = $1", [
      sessionId,
    ]);
    ...
    const userMessage = userMsgRes.rows[0] as ChatMessage;
    ...
    const aiMsgRes = await query(
      "INSERT INTO chat_messages (session_id, user_id, role, content) VALUES ($1, $2, $3, $4) RETURNING *",
      [sessionId, userId, "assistant", aiText]
    );
    const aiMessage = aiMsgRes.rows[0] as ChatMessage;
    ...
    if (isFirst) {
      try {
        ...
        await query("UPDATE chat_sessions SET title = $1 WHERE id = $2", [newTitle, sessionId]);
      } catch (e) {
